...
@Component({
  ...
  template: `
    <div class="h-[56px] bg-white border-b border-gray-200 px-4 flex items-center justify-between flex-shrink-0 shadow-sm">
      ...
      <div class="flex items-center gap-4">
        ...
        <div class="flex items-center gap-2">
          ...
          <span class="font-bold text-[#0D2441] text-[18px] tracking-tight leading-none">Invoice <span class="text-[#10B981]">360</span></span>
        </div>

        <!-- Keyboard Icon Button -->
        <button class="hidden sm:flex w-9 h-9 border border-gray-200 rounded-full items-center justify-center text-gray-500 hover:bg-gray-50 transition-colors">
          <svg class="w-[18px] h-[18px]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <rect x="2" y="6" width="20" height="12" rx="2" stroke-width="1.5"/>
            <path d="M6 10h.01M9 10h.01M12 10h.01M15 10h.01M18 10h.01M7.5 14h9" stroke-width="1.5" stroke-linecap="round"/>
          </svg>
        </button>

        <!-- Time Badge -->
        <div class="hidden md:flex items-center gap-1 text-[13px] text-[#10B981] font-semibold bg-emerald-50 border border-emerald-100 px-3 py-1 rounded-full">
          {{ currentTime() }}
        </div>
      </div>
      ...
      <div class="flex items-center gap-2">
        ...
        <!-- User Avatar -->
        <div class="w-8 h-8 rounded-full bg-[#10B981] text-white flex items-center justify-center text-[14px] font-bold select-none">
          R
        </div>
